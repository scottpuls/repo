...
from electrum_trc.bitcoin import (TYPE_ADDRESS, int_to_hex, var_int,
                                   b58_address_to_hash160,
                                   hash160_to_b58_address)
from electrum_trc.bip32 import BIP32Node
from electrum_trc.i18n import _
from electrum_trc.keystore import Hardware_KeyStore
from electrum_trc.transaction import Transaction
from electrum_trc.wallet import Standard_Wallet
from electrum_trc.util import bfh, bh2u, versiontuple, UserFacingException
from electrum_trc.base_wizard import ScriptTypeNotSupported
...
class btchip_terracoin(btchip):
    ...
    def getTrustedInput(self, transaction, index):
        ...
        if transaction.extra_data:
            # Terracoin DIP2 extra data: By appending data to the 'lockTime' transfer we force the device into the
...
class Ledger_Client():
    def __init__(self, hidDevice):
        self.dongleObject = btchip_terracoin(hidDevice)
        ...
        if not self.preflightDone:
            try:
                ...
                if (e.sw == 0x6d00 or e.sw == 0x6700):
                    raise UserFacingException(_("Device not in Terracoin mode")) from e
                raise e
