...
FEERATE_STATIC_VALUES = [1500, 3000, 5000, 10000,
...
class SimpleConfig(Logger):
    ...
    def electrum_path(self):
        ...
        self.logger.info(f"electrum-trc directory {path}")
        return path
    ...
    def convert_version_3(self):
        ...
        if isinstance(base_unit, str):
            ...
            map_ = {'trc':8, 'mtrc':5, 'utrc':2, 'sat':0}
...
def read_user_config(path):
    """Parse and store the user config settings in electrum-trc.conf into user_config[]."""
    if not path:
        return {}
