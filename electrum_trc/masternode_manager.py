...
class MasternodeManager(object):
    ...
    def populate_masternode_output(self, alias):
        ...
        if mn.collateral_key and mn.vin.get('address') and mn.vin.get('value') == 5000 * bitcoin.COIN:
            return
    ...
    def get_masternode_outputs(self, domain = None, exclude_frozen = True):
        ...
        correct_amount = lambda d: d['value'] == 5000 * bitcoin.COIN

        # Valid outputs have a value of exactly 5000 TRC and
    ...
    def check_can_sign_masternode(self, alias):
        ...
        if mn.vin.get('value', 0) != bitcoin.COIN * 5000:
            raise Exception('Masternode requires a collateral 5000 TRC output.')
