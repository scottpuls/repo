...
class CoinCap(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        # CoinCap rates API is broken for Terracoin.  Use history API instead.
        """
        json = await self.get_json('api.coincap.io', '/v2/rates/terracoin/')
        return {'USD': Decimal(json['data']['rateUsd'])}
        """
        json = await self.get_json('api.coincap.io',
                                      '/v2/assets/terracoin/history?interval=d1&limit=2000')
...
class CoinGecko(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        # CoinGecko exchange_rates API is unavailable for Terracoin.  Use coins API instead.
        """
        json = await self.get_json('api.coingecko.com', '/api/v3/exchange_rates')
        return dict([(ccy.upper(), Decimal(d['value']))
                     for ccy, d in json['rates'].items()])
        """
        json = await self.get_json('api.coingecko.com', '/api/v3/coins/terracoin')
    ...
    async def request_history(self, ccy):
        history = await self.get_json('api.coingecko.com',
                                      '/api/v3/coins/terracoin/market_chart?vs_currency=%s&days=max' % ccy)
