...
from electrum_trc import terracoin_tx
from electrum_trc.bitcoin import COIN, is_b58_address
from electrum_trc.terracoin_tx import TxOutPoint, service_to_ip_port
from electrum_trc.protx import ProTxMN, ProTxService, ProRegTxExc
...
class OutputsList(QListWidget):
    '''Widget that displays available 5000 TRX outputs.'''
...
class ImportLegacyWizardPage(QWizardPage):
    ...
    def update_lmn_data(self, current):
        ...
        if prevout_hash:
            val_trc = '%s TRC' % (value/COIN) if value else ''
            self.collateral_val.setText(val_trc)
...
class BlsKeysWizardPage(QWizardPage):
    ...
    def initializePage(self):
        ...
        if start_id in parent.UPD_ENTER_PAGES:
            self.setSubTitle(_('Regenerate BLS keypair, setup terracoind'))
            if not self.bls_priv.text():
                self.bls_priv.setText(new_mn.bls_privk)
                self.bls_pub.setText(new_mn.pubkey_operator)
        else:
            self.setSubTitle(_('Generate BLS keypair, setup terracoind'))
    ...
    def generate_bls_keypair(self):
        ...
        self.bls_info_label.setText(_('BLS keypair generated. Before '
                                      ...
                                      'line to ~/.terracoincore/terracoin.conf and '
                                      'restart masternode:'))
...
class CollateralWizardPage(QWizardPage):
    ...
    def __init__(self, parent=None):
        ...
        self.not_found = QLabel('No 5000 TRC outputs were found.')
    ...
    def scan_for_outputs(self):
        ...
        coins = wallet.get_utxos(domain=None, excluded_addresses=excluded,
                                 mature_only=True, confirmed_only=True)
        coins = list(filter(lambda x: (x['value'] == (5000 * COIN)), coins))
...
class Dip3MasternodeWizard(QWizard):
    ...
    def validate_collateral(self, outpoint, addr, value, skip_alias=None):
        ...
        if not value == 5000 * COIN or not value == c_vin['value']:
