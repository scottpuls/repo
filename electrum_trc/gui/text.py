...
import electrum_trc
from electrum_trc.terracoin_tx import SPEC_TX_NAMES
from electrum_trc.util import format_satoshis
from electrum_trc.bitcoin import is_address, COIN, TYPE_ADDRESS
from electrum_trc.transaction import TxOutput
from electrum_trc.wallet import Wallet
from electrum_trc.storage import WalletStorage
from electrum_trc.network import NetworkParameters, TxBroadcastError, BestEffortRequestFailed
from electrum_trc.interface import deserialize_server
from electrum_trc.logging import console_stderr_handler
...
class ElectrumGui:
    ...
    def __init__(self, config, daemon, plugins):
        ...
        if not storage.file_exists():
            print("Wallet not found. try 'electrum-trc create'")
            exit()
    ...
    def network_dialog(self):
        ...
        if out:
            ...
            if out.get('server') or out.get('proxy'):
                proxy = electrum_trc.network.deserialize_proxy(out.get('proxy')) if out.get('proxy') else proxy_config
