...
from electrum_trc.util import profiler, parse_URI, format_time, InvalidPassword, NotEnoughFunds, Fiat
from electrum_trc import bitcoin
from electrum_trc.transaction import TxOutput, Transaction, tx_from_str
from electrum_trc.util import send_exception_to_crash_reporter, parse_URI, InvalidBitcoinURI
from electrum_trc.paymentrequest import PR_UNPAID, PR_PAID, PR_UNKNOWN, PR_EXPIRED
from electrum_trc.plugin import run_hook
from electrum_trc.wallet import InternalAddressCorruption
from electrum_trc import simple_config
...
class CScreen(Factory.Screen):
    ...
    def load_screen(self):
        self.screen = Builder.load_file('electrum_trc/gui/kivy/uix/ui_screens/' + self.kvname + '.kv')
...
class HistoryScreen(CScreen):
    ...
    def get_card(self, tx_hash, tx_type, tx_mined_status, value, balance):
        ...
        icon = "atlas://electrum_trc/gui/kivy/theming/light/" + TX_ICONS[status]
...
class SendScreen(CScreen):
    ...
    def do_save(self):
        ...
        from electrum_trc.paymentrequest import make_unsigned_request, PaymentRequest
    ...
    def do_send(self):
        if self.screen.is_pr:
            ...
            if not address:
                self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a Terracoin address or a payment request'))
                return
            if not bitcoin.is_address(address):
                self.app.show_error(_('Invalid Terracoin Address') + ':\n' + address)
                return
            try:
...
class ReceiveScreen(CScreen):
    ...
    def get_URI(self):
        from electrum_trc.util import create_bip21_uri
    ...
    def do_share(self):
        uri = self.get_URI()
        self.app.do_share(uri, _("Share Terracoin Request"))
