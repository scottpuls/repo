...
To create a keystore run the following command:
    ...
    mkdir ~/.jks && keytool -genkey -v -keystore ~/.jks/keystore \
        -alias electrum.terracoin.org -keyalg RSA -keysize 2048 \
        -validity 10000
...
Manual signing:
    ...
    jarsigner -verbose \
        ...
        -sigfile trc-electrum \
        -keystore ~/.jks/keystore \
        Electrum_TRC-3.0.6.1-release-unsigned.apk \
        electrum.terracoin.org
...
Manual zip aligning:
    ...
    android-sdk-linux/build-tools/27.0.3/zipalign -v 4 \
        Electrum_TRC-3.0.6.1-release-unsigned.apk \
        Terracoin-Electrum-3.0.6.1-release.apk
...
SDIST_NAME_PATTERN = re.compile('^Terracoin-Electrum-(.*).tar.gz$')
SDIST_DIR_TEMPLATE = 'Terracoin-Electrum-{version}'
PPA_SOURCE_NAME = 'electrum-trc'
...
JKS_ALIAS = 'electrum.terracoin.org'
...
UNSIGNED_APK_PATTERN = re.compile('^Electrum_TRC(_Testnet)?-(.*)-release-unsigned.apk$')
SIGNED_APK_TEMPLATE = 'Terracoin-Electrum{testnet}-{version}-release.apk'
