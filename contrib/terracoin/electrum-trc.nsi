...
;Variables
  ...
  !define PRODUCT_NAME "Terracoin Electrum"
  !define PRODUCT_NAME_NO_SPACE "Terracoin-Electrum"
  !define PREV_PROD_NAME "Electrum-TRC"
  !define PREV_PROD_NAME2 "Terracoin-Electrum"
  !define PRODUCT_WEB_SITE "https://github.com/terracoin/electrum-trc"
...
;Interface Settings
  ...
  !define MUI_ICON "electrum_trc\gui\icons\electrum-trc.ico"
...
Section "${PRODUCT_NAME}" SectionDE
  ...
  ${If} ${PREVINSTDIR} != ""
    ...
    DeleteRegKey HKCU "Software\Classes\trc"
  ...
  ${If} ${PREVINSTDIR} != ""
    ...
    DeleteRegKey HKCU "Software\Classes\terracoin"
  ...
  File /r "dist\electrum-trc\*.*"
  File "electrum_trc\gui\icons\electrum-trc.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-trc-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-trc-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-trc-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-trc-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-trc-${PRODUCT_VERSION}.exe" 0


  ;Links trc: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\terracoin" "" "URL:terracoin Protocol"
  WriteRegStr HKCU "Software\Classes\terracoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\terracoin" "DefaultIcon" "$\"$INSTDIR\electrum-trc.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\terracoin\shell\open\command" "" "$\"$INSTDIR\electrum-trc-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-trc.ico"
...
;--------------------------------
;Descriptions
LangString DESC_DE ${LANG_ENGLISH} "Terracoin Electrum Wallet"
